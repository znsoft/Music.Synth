...
static int go(void);
static int addprc();
int WinMainCRTStartup(){go();return 0;}
...
float e=1.0594630943593f;//pow(2,(1.f/12.f));
	int i;float n=23.0f;for(i=0;i<227;i++){
n*=e;
notetab[i]=lround(n);}}
//--------------------------------------------
short sintab[1025];//------------------------------------------------
int hpfb[256];
void initsintab(){for (int i=0;i<=1024;i++) sintab[i]=lround(32760*(float)sin(i*6.28f/1023));
	makenotetab();
for(int i=0;i<255;i++)hpfb[i] = lround(32760*exp(-2.0*3.14*i/255));}
//---------------------------------
//---------------------------------
int rnd,yne;
...
if (indx<=1||indx>=w) {indx=w;yne=rnd&1023;};
indx--;return sintab[yne];}
//--------------------------
...
{  short nowamp;
indx+=freq;
if (indx>=frequency)indx=abs(frequency-indx);
unsigned long rise=1+phase*frequency/120;
if (indx<=rise){nowamp=32000-64000*indx/rise;}
	else{nowamp=32000-64000*(frequency-indx)/(frequency-rise);}
return nowamp;}
//--------------------------------------
...
int freq,freq2,i,y,x,z,n,u,dff=0,df=0,fud=-1;long b,v,zl,dl=abs(delay)*50;
	int fuzz=0;b=indx;v=b;
	if(hpfreq>100)fuzz=hpfreq-100;
	if(frequency==44000)res=res>>1;
//if(version==4)		{
		freq=notetab[note]+offset-63;	freq2=(notetab[note2])*(note2!=0);
	...
	for(i=0;i<=li;i++){x=*buf; 
		...
		if(osc==4)	y=((phase<<2)+fdcsaw-(f*(indx++)/4)&fsaw)*fmulsaw;//my fast binary saw generator  /|/|/|/|
		if(osc==5)	y=(fdcsaw-(f*(indx++)/4)&fsaw)*(fdcsaw-((f+phase)*indx/4)&fsaw)/fdivsaw; //fast dissonance for drum and bass
		if(osc==6)	{indx=b;y=gensawsample(f+phase,63);b=indx;indx=v;y=(y*gensqrsample(f,0))/32000;v=indx;	}
...
int i,x,y,u,z,s,l;wchar_t t[100];
addprc();
	for(i=0;i<=3;i++){
		for(z=0;z<=maxinstr;z++){	for(u=0;u<16;u++)if((signed char)instr[z][u]<0){s=seq[playseq][abs(instr[z][u]+1)]-1;
				if(s<0)editinst[u]=0; else editinst[u]=patterns[s][notenum];}
												else editinst[u]=instr[z][u];bhf1=0;bhf0=0;
												buf0=0,buf1=0;indx=0;l=editinst[5]*(beatlen>>4);
												if(editinst[5]==0&&editinst[1]){l=beatlen;indx=string[z].indx;buf0=string[z].buf0;buf1=string[z].buf1;bhf0=string[z].b0;bhf1=string[z].b1;}
	if(editinst[1])fillnote(buf+i*beatlen,l,editinst[0],editinst[1],editinst[2],editinst[3],editinst[4],editinst[6],editinst[7],editinst[8],editinst[9],editinst[10],editinst[11],editinst[12],editinst[13],editinst[14],editinst[15],z);	
				string[z].indx=indx;string[z].buf0=buf0;string[z].buf1=buf1;string[z].b0=bhf0;string[z].b1=bhf1;		}
	notenum++;	if(!(notenum&=0xf))playseq++;
...
int i;
fdivsaw=0x1ff;fsaw=0x3fff;if(frequency==22000){fsaw=fsaw>>1;fdivsaw=fdivsaw>>1;}	fdcsaw=fsaw>>1; fmulsaw=10/((frequency==22000)+1);
beat=15000/bpm;
...
r=1300; // raidus
ix=x2-x1;ix*=ix;iy1=y2-y1;iy1*=iy1;
z=(ix+iy1);

	for(y=sy;y--;){
		dy1=y1-y;iy1=dy1*dy1;
		dy2=y2-y;iy2=dy2*dy2;
		for(x=sx;x--;){
			dx1=x1-x;ix=dx1*dx1;i1=(ix+iy1);
			dx2=x2-x;ix=dx2*dx2;i2=(ix+iy2);
			if(z){c=i1*i2/z;cc=(i1>>7)*(i2>>7);// gravity
				r=1300+50000/z;}
						if (c>r||cc>500) c=pnt(b,x,y); else 
				if(z){
				c|=11;
				if(c>(r-200))l=0x1505; // blind
				
				kx=k*dx2*dx1/z;ky=k*dy2*dy1/z;
				xx=x+kx;yy=y+ky;l=0;
				c=pnt(bb,xx,yy);if(kx)if((ky/kx)>0)c|=l;}
...
int hx[3000];
	...
	d=(y1-850)<<fovs;
	for(y=yl;y--;){//nebo
			if(y>yl)z=e/(y-yl);else z=d/(yl-y);		
		 for(x=sx;x--;){
			v=z;
			u=(z*(xi-x))>>fovs;
			u-=x1;
...
for(x=sx;x--;)hx[x]=sy;
	for(y=sy;y>yi;y--){
			if(y>yi)z=e/(y-yi);else z=d/(yi-y);		
		 for(x=sx;x--;){
			v=z;
			u=(z*(xi-x))>>fovs;
			u-=x1;
			...
			g=c;
			s=hx[x];l=524-z;
			if(z<1500)g=y-((l*g)>>18)+50;else g=s;
			c=txtr[2][f];
			//if(c<300)c|=txtr[1][f];
			if(s>g){l=g;
			for(;l<s;l++)
			if(sy>l&&l>0)fb[x+l*sx]=c;
			hx[x]=g;}
}}}
//--------------------------------
...
int xt;
int xb=x-xs/2;if(xb<0)xb=0;
int yb=y-ys/2;if(yb<0)yb=0;
LPWORD f=a+xb+yb*sx;
for(;ys--;)for(xt=xs;xt--;)f[ys*sx+xt]|=c;}
//..........-----=========......
void dbox(LPWORD a,int x,int y,int xs,int ys,WORD c){
LPWORD f=a+x+y*sx;
for(;ys--;f+=sx)memset(f,c,xs*2);}
//--------------
void dchr2(LPWORD fb,int u,int v,char c,WORD cc){
char x,y,z,mm=0;
LPWORD f=fb+u+v*sx;
for(y=0;y<8;y++){z=t[c][y];
for(x=7;x--;z<<=1)if(z&128){f[x+y*sx]=cc;mm=1;}else if(mm){mm=0;f[x+y*sx]=0xffff-cc;};
}}
//----------------
...
LPWORD f=fb+u+v*sx;
for(y=8;y--;){z=t[c][y];
for(x=7;x--;z<<=1)if(z&128)f[x+y*sx]=cc;
}}
//---------------------
void prnt(char *t,LPWORD f,int x,int y,WORD c){
char r=*(t++);
while(r!=0){
if(r<maxsym)dchr(f,x,y,r,c);r=*(t++);x+=8;
}}
//i----------------------
void prnt2(char *t,LPWORD f,int x,int y,WORD c){
char r=*(t++);
while(r!=0){
if(r<maxsym)dchr2(f,x,y,r,c);r=*(t++);x+=8;
}}
//------------------------
int addprc(){
char str[255];
...
//-------------------
int randi(int rnd){
rnd+=GetTickCount()+an++;return rnd;}//^an=sierpinski
//++++++++++
...
for(x=r;x--;){
c=f[x];
l=(x+txs*t+t)&r;c+=f[l];l=(l-t)&r;c+=f[l];l=(l-t)&r;c+=f[l];
l=(x-txs*t+t)&r;c+=f[l];l=(l-t)&r;c+=f[l];l=(l-t)&r;
c+=f[l];l=(x-t)&r;c+=f[l];l=(x+t)&r;
c+=f[l];c=c/9;f[x]=c;}	}
		}
//----------------
void to5b(LPWORD f,int t,char s){
int r=((t*t)-1);short c;
for(;r--;){c=f[r]&0x1f;f[r]=(c<<5)|(c<<11)|c;}
}
//++++++++++
int mndb3(LPWORD f,int u,int v){
	...
	s=(float)zc/5.0f;
	for(x=0;x<=u;x++)d[x]=(float)(x-xi)/zi-xf;
	for(y=0;y<v;y++){
	addprc();
	b=(float)(y-yi)/zi-yf;
	for(x=0;x<u;x++){
...
fb = (LPWORD) 
screen_rfbi.pFramePointer;
bb= (LPWORD) LocalAlloc(0,2*sx*sy);

int p;
for(p=3;p--;)txtr[p]=(LPWORD) LocalAlloc(0,2*txs*txs);
...
prnt(str,fb,1,59,0xffff);
	...
	unsigned int i;
prc=0;
for(i=0;i<=(maxseq*4);i++)fillshort((short*)(b1+BufferSize*i));
...
fff=GetTickCount(); f=fff-f;if(f)fps=1000/f;f=fff;
r=py/3+40;
z1=fff/r;z2=(fff/r)>>1;
testtan2(bb);
